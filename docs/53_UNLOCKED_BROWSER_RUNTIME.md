...
When `BROWSER_RUNTIME_BENCH_STRICT` is unset, threshold misses are emitted as artifact fields but do not fail the command or normal release gates. Set `BROWSER_RUNTIME_BENCH_STRICT=true` for a device-specific lane that should fail on performance regressions. Set `BROWSER_RUNTIME_BENCH_REQUIRE_MTP_ACCELERATION=true` to run each prompt twice, once with draft verification and once target-only, then fail unless `BROWSER_RUNTIME_BENCH_MIN_MTP_ACCEPTANCE_RATE` and `BROWSER_RUNTIME_BENCH_MIN_MTP_NET_SPEEDUP` are cleared. Local `release:gate` no longer promotes installed Qwen assets into real-Qwen mode by default. Set `RELEASE_REQUIRE_UNLOCKED_MODEL=true` for an explicit configured Kernel Lab release gate, or set `RELEASE_AUTO_DETECT_UNLOCKED_MODEL=true` only when installed local assets should promote that lane. In the explicit lane, configured manifest, manifest SHA, full runtime profile, Qwen math/parity, KV decode reuse, `browser-vector` memory, WebGPU backend preference, and a completed strict browser-preview proof are all expected. Add `RELEASE_REQUIRE_MTP_ACCELERATION=true` when speculative decoding must prove a real speedup; otherwise MTP remains target-only/off for the production speed claim.
