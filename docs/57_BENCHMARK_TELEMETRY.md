...
`pnpm eval:v12-production` is the strict production archive command for telemetry-backed release claims. It requires the saved hosted benchmark artifact, includes hosted benchmark proof, and writes `.artifacts/evals/v12-production-archive-latest.json`. With `RELEASE_REQUIRE_V12_PRODUCTION=true`, `pnpm release:gate` requires that archive and validates its backend-specific proof fields before reporting production release proof.
...
For GitHub-hosted release verification, the manual **V12 Production Proof** workflow accepts either `HOSTED_BENCHMARK_ARTIFACT_JSON` or `HOSTED_BENCHMARK_ARTIFACT_URL` through workflow inputs. Internally it runs:
