...
This writes `.artifacts/evals/hosted-benchmark-proof-latest.json`. Use `RELEASE_REQUIRE_HOSTED_BENCHMARK_PROOF=true` when `pnpm release:gate` should fail unless the saved browser artifact proves proof schema version `2`, the compiled backend, `modelId=Qwen3-0.6B-q4f16_1-MLC`, a public HTTPS `deployUrl` matching the expected hosted deploy URL, matching Backend Broker selected model, Backend Broker selection, deploy/Kernel Lab/fallback role-boundary evidence, concrete run-level grounded memory evidence, exact output, speed floor, successful benchmark telemetry submission, and no CPU fallback. Concrete memory evidence means the run carries expected memory ids, retrieved memory ids, context-included memory ids, and a retrieval rank for the grounded answer. Telemetry evidence means the browser benchmark summary reports telemetry requested, configured, submitted, and a successful HTTP status.
