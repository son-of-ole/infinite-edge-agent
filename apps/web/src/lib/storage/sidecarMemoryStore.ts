...
export class SidecarMemoryStore implements MemoryStore, MemorySnapshotStore, RuntimeTraceStore, RuntimeTraceSnapshotStore, GacMemoryStore {
  ...
  async upsert(chunks: MemoryChunk[]): Promise<void> {
    ...
    if (!response.ok) throw new Error(`Memory sidecar upsert failed: ${response.status}`);
  }
  ...
  async search(queryEmbedding: number[], options: MemorySearchOptions = {}): Promise<MemorySearchHit[]> {
    ...
    if (!response.ok) throw new Error(`Memory sidecar search failed: ${response.status}`);
  ...
  async clear(): Promise<void> {
    ...
    if (!response.ok) throw new Error(`Memory sidecar clear failed: ${response.status}`);
  }
  ...
  async deleteMemory(options: MemoryDeleteOptions): Promise<number> {
    ...
    if (!response.ok) throw new Error(`Memory sidecar delete failed: ${response.status}`);
  ...
  async listMemoryChunks(options: { sessionId?: string; limit?: number; tenantId?: string; cellId?: string } = {}): Promise<MemoryChunk[]> {
    ...
    if (!response.ok) throw new Error(`Memory sidecar export failed: ${response.status}`);
  ...
  async importMemoryChunks(chunks: MemoryChunk[]): Promise<void> {
    ...
    if (!response.ok) throw new Error(`Memory sidecar import failed: ${response.status}`);
  }
  ...
  async writeRuntimeTrace(trace: RuntimeTrace): Promise<void> {
    ...
    if (!response.ok) throw new Error(`Memory sidecar trace write failed: ${response.status}`);
  }
  ...
  async listRuntimeTraces(options: RuntimeTraceListOptions = {}): Promise<RuntimeTrace[]> {
    ...
    if (options.cellId) params.set("cellId", options.cellId);
    if (options.limit !== undefined) params.set("limit", String(options.limit));
    const response = await this.request(`/runtime/traces?${params.toString()}`, { method: "GET" });
    if (!response.ok) throw new Error(`Memory sidecar trace read failed: ${response.status}`);
  ...
  async writeMemoryRepresentatives(records: MemoryRepresentativeRecord[], lineage?: MemoryLineageRecord[]): Promise<GacWriteResult> {
    ...
    if (!response.ok) throw new Error(`Memory sidecar GAC representatives write failed: ${response.status}`);
  ...
  async listContextPackTraces(options: GacListOptions = {}): Promise<ContextPackTraceRecord[]> {
    ...
    if (!response.ok) throw new Error(`Memory sidecar GAC context pack trace read failed: ${response.status}`);
  ...
  private async writeGacRecords<T>(path: string, records: T[]): Promise<GacWriteResult> {
    ...
    if (!response.ok) throw new Error(`Memory sidecar GAC write failed for ${path}: ${response.status}`);
    return response.json() as Promise<GacWriteResult>;
  }
  ...
  private async listGacRecords<T>(path: string, options: GacListOptions): Promise<T[]> {
    ...
    if (!response.ok) throw new Error(`Memory sidecar GAC read failed for ${path}: ${response.status}`);
