...
For GitHub-hosted release verification, the manual **V12 Production Proof** workflow accepts exactly one of `HOSTED_BENCHMARK_ARTIFACT_JSON`, `HOSTED_BENCHMARK_ARTIFACT_BASE64`, or `HOSTED_BENCHMARK_ARTIFACT_URL` through workflow inputs. URL-sourced artifacts must use HTTPS; for local or private artifacts, use pasted JSON or base64 instead. Internally it runs:
