...
`pnpm eval:v12-production` is the strict production archive command for telemetry-backed release claims. It requires the saved hosted benchmark artifact, includes hosted benchmark proof, requires Backend Broker selection evidence, binds backend readiness to that proof, and writes `.artifacts/evals/v12-production-archive-latest.json`. With `RELEASE_REQUIRE_V12_PRODUCTION=true`, `pnpm release:gate` requires that archive and validates its backend-specific proof fields before reporting production release proof.
