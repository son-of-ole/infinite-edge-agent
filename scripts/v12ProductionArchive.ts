...
export function buildV12ProductionArchiveArtifact(input: {
  ...
  return {
    ...
    summary: {
      ...
      v12ProductionTelemetryRequested: hostedProof?.benchmarkTelemetryRequested ?? false,
      v12ProductionTelemetryConfigured: hostedProof?.benchmarkTelemetryConfigured ?? false,
      v12ProductionTelemetrySubmitted: hostedProof?.benchmarkTelemetrySubmitted ?? false,
      v12ProductionTelemetryStatus: hostedProof?.benchmarkTelemetryStatus ?? null,
      v12ProductionGpuLabelEvidencePassed: hostedProof?.gpuLabelEvidencePassed ?? false,
      v12ProductionGpuVendor: hostedProof?.gpuVendor ?? null,
      v12ProductionGpuArchitecture: hostedProof?.gpuArchitecture ?? null,
      v12ProductionGpuDevice: hostedProof?.gpuDevice ?? null,
      v12ProductionGpuDescription: hostedProof?.gpuDescription ?? null,
      v12ProductionWebGlRenderer: hostedProof?.webglRenderer ?? null,
      ...
      v12ProductionBrokerProofRequirementCount: hostedProof?.brokerProofRequirements.length ?? 0,
      v12ProductionBrokerRequiresBackendTrace: hostedProof?.brokerProofRequirements.includes("backend_trace") ?? false,
      v12ProductionBrokerRequiresMemoryGrounding: hostedProof?.brokerProofRequirements.includes("memory_grounding") ?? false,
