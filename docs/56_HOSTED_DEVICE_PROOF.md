...
It writes the hosted profile, backend readiness matrix, shared runtime readiness, v12 readiness bundle, v12 production workflow preflight, and `.artifacts/evals/v12-readiness-suite-latest.json` with the same timestamp. If `HOSTED_BENCHMARK_ARTIFACT_PATH` is set, it also validates and writes `hosted-benchmark-proof-latest.json` as part of the same suite. Set `HOSTED_BENCHMARK_REQUIRE_SOURCE_BOUND=true` when the suite should require source-bound verification against `HOSTED_BENCHMARK_EXPECTED_GIT_SHA` or `GITHUB_SHA`; the suite summary exposes this as `v12SuiteHostedBenchmarkProofSourceBoundRequired` and keeps `v12SuiteProductionWorkflowPreflightPassed` visible. Archive this suite next to hosted Chrome/Edge benchmark evidence when making a backend-specific deploy-readiness claim.
