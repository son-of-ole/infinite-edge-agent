...
For v12 production proof, the browser benchmark artifact carries this deployment SHA as `v12ProductionProofSourceGitSha` and carries the hosted origin as `deployUrl`. Release verification should set `HOSTED_BENCHMARK_EXPECTED_GIT_SHA=<deployment-sha>` so stale hosted artifacts from another build fail instead of being reused. `deployUrl` must be a public HTTPS origin; local, loopback, link-local, private-network, and non-HTTPS origins are rejected by hosted proof gates.
