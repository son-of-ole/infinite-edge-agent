...
`pnpm eval:v12-readiness` writes `.artifacts/evals/v12-readiness-bundle-latest.json`, combining hosted profile, backend matrix, shared-runtime proof, and the explicit `model_registry_alignment` requirement into one final-state artifact. The summary exposes `v12ModelRegistryAligned`, `v12PublicDeployOptionCount`, and `v12PublicKernelLabOptionCount`. Use `RELEASE_REQUIRE_V12_READINESS=true` to require that bundle independently.
